...
### Added
- **P2 报告三件套（2026-09-27）：重复文件 / 启动项 / TM 快照**。① `dupes` 子命令（+`GET /api/dupes` 后台构建 + 看板面板）：大小 → 64KB 头哈希 → 全量 SHA-256 四级渐进，硬链接（dev+ino）不算重复，.git 排除，阈值 10MB；报告性质（选哪份保留是人决策）；首次真实运行 **25 组 / 2.1G**（hotnode-sync-hub 备份树同一 .blend 存 5 份）。② `launch_items()` + `GET /api/launch`：第三方 LaunchAgents/LaunchDaemons 只读解析，com.apple.* 过滤，刻意不接删除（归 launchctl）。③ TM 快照：`tmutil listlocalsnapshots` 解析（**坑：输出首行是标题，须过滤**，否则被当快照名）；删除仅接受日期格式名 + 逐字 `confirm:"DELETE SNAPSHOT"`，`com.apple.os.update-*` 回滚点代码级拒绝。看板新增「报告」视图（第 6 视图）承载三面板；快照删除强确认弹层；dupes building 态 5 秒轮询。单测 65 → 70，DOM 67 → 79。
- **P1 第二批（2026-09-27）：浏览器缓存 / 安装包清扫 / iOS 备份报告**。① `browser-cache`：App Support 整体 prune 造成的盲区——Chromium 系（Chrome/Edge/Brave/Vivaldi/Arc）profile 缓存 + Firefox cache2，仅白名单子目录（Code Cache/GPUCache/着色器缓存等 = safe；Chrome 内置 AI 模型仓 = aggressive），**Service Worker CacheStorage/ScriptCache 永不碰**（那是站点数据不是缓存），浏览器运行中整组跳过。② `installer`：Downloads 的 .dmg/.pkg/.mpkg/.iso/.xip（depth 2）+ ZIP 载荷校验（纯 Python zipfile，无子进程），Mole 的 Desktop/Documents/Public/iCloud 刻意不扫，全 manual。③ `ios-backup`：MobileSync/Backup 只读清单，全 manual；MobileSync 是 TCC 保护目录，实测无权限 iterdir 直接抛异常**把整个 scan 干崩**——已降级为类别缺席（与 web 面板 trash API 同一处理）。单测 60 → 65。
- **P1 三件套（2026-09-27）：大文件视图 / 系统废纸篓管理 / 磁盘树图**。① 新类别 `large-files`（scan_roots 内 >100MB 或 >12月&>10MB 的单文件，全 manual 永不自动勾选，跳过已被前序 pass 覆盖的路径与符号链接，上限 200 条；首次真实运行 10 项 2.1G 全为真目标）。② `GET /api/trash` 升级为 quarantine + system 两块（旧字段保留），system 列 `~/.Trash` 顶层项且**默认保留隔离区**；`POST /api/trash/empty-system` 需逐字 `confirm:"EMPTY TRASH"`；`~/.Trash` 是 TCC 保护目录，读不了时降级 `available:false` 而非 500，看板给出授权指引。③ 看板 overview 新增 squarified treemap（纯 SVG，top 12 + 其他），历史视图废纸篓拆「清理隔离区 / 系统废纸篓」两段 + 强确认弹层；顺手修复 `renderTrash()` 因 `#trash-info` 不存在而早退、隔离区面板从未被填充的既有 bug。单测 54 → 60，DOM 检查 59 → 67。

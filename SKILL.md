...
- **`skilldo update` rebuilds the central directory from the repository**, deleting every file the repo does not track (`remove_dir_all` + re-clone). Measured on 2026-09-23, exactly four local-only entries are lost and nothing else — tracked content comes back byte-for-byte:
  - `.workbuddy` (the notes symlink) → re-link with `ln -s ~/.codex/logs/mac-dev-cleanup/.workbuddy ~/.skillshub/mac-dev-cleanup/.workbuddy`
  - `config_data.js`, `dashboard_data.js` and `dashboard.html` → regenerated by the next `scan` (the first two carry the scan data, the third is the template's build output). The dashboard is therefore unavailable until a scan runs — that is the intended cost of keeping the generated page out of the repository.
...
7. **Check that the docs still match after collapsing a pair**: structural changes leave prose behind. Grep the repo for the deleted or renamed path and for any claim about how the artifact is produced (e.g. "the build inlines the data") — a stale sentence that contradicts the code is worse than no sentence.
